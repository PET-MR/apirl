classdef PETDataClass < handle
        ...
        function [type] = Which(PETData,path)
            ...
            for i = 3:length(listing)
                ...
                if strcmpi(ext,'.IMA') || strcmpi(ext,'.PDT') || length(ext) > 5
                    j_d = j_d + 1;
                elseif strcmpi(ext,'.mhdr') && ~isempty(strfind(name,'.s')) && isempty(strfind(name,'uncomp'))
                    j_si = j_si + 1;
                elseif (strcmpi(ext,'.mhdr') && ~isempty(strfind(name,'.s'))) && ~isempty(strfind(name,'uncomp'))
                    j_sui = j_sui + 1;
    ...
    methods (Access = public)
        ...
        function  P = Prompts(PETData,frame)
            if nargin==1 && ((PETData.isSinogram)>1 || (PETData.isListMode && (PETData.NumberOfFrames>1) )),
                frame =1;
        ...
        function mul = AN(PETData,frame)
            mul = PETData.ACF(frame) .* PETData.NCF(frame);
            mul = 1./mul;
            mul(isinf(mul)) = 0;
        end
