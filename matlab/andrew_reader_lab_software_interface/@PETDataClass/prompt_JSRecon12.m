...
if ~exist(Root_path,'dir') && ~reFraming % if is reframing iw shouldnt be necessary to acces to this one.
    [status,message] = system([PETData.SoftwarePaths.e7.JSRecon12 ' ' FolderName]);
    if status
...
% Get some info from sinogram:
[info] = getInfoFromSiemensIntf([PETData.DataPath.emission(i).n(1:end-4) 's.hdr']);
...
if ~isempty(LM)
    ...
    if ~isempty(PETData.FrameTimePoints)
        % 1 frame
        PETData.NumberOfFrames = 1;
        % The frames from 0 to the total time:
        PETData.FrameTimePoints = [0 info.ImageDurationSec];
    else
        % Remove the frames that end later the acquisition.
        indicesPostTime = PETData.FrameTimePoints > info.ImageDurationSec;
        PETData.FrameTimePoints(indicesPostTime) = [];
        PETData.FrameTimePoints = [PETData.FrameTimePoints info.ImageDurationSec];
    end
