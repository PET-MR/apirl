...
if (checkdict)
  if (isempty(G))
    atomnorms = sqrt(sum(D.*D));
  else
    atomnorms = diag(G);
  end
  if (any(abs(atomnorms-1) > 1e-1))
      save dic D
      D(isnan(D)) = 1e-5;
      D(isinf(D)) = 1e-5;
      id= abs(sqrt(sum(D.^2))-1) > 1e-1;
      
      D(:,id) = 1e-5./sqrt(sum((1e-5*ones(1,size(D,1))).^2));
      display('Dictionary columns must be normalized to unit length');
  end
end
