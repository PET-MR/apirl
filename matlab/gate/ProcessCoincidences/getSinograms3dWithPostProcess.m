...
%  *********************************************************************
%	            GENERACIÓN DE SINOGRAMA 3D A PARTIR DE SIMULACIÓN GATE
...
function sinograms3D = getSinograms3D(outputPath, structSimu, structSizeSino3D, resEspacialFwhm_mm, rebinningMethod, zonaMuerta_mm, graficarOnline)
            ...
            for r = 1 : numel(resEspacialFwhm_mm)   % Por ahora considero que la resolución espacial es la misma en los dos sentidos.
                for zm = 1 : size(zonaMuerta_mm, 1)
                    ...
                    switch(rebinningMethod)
                        case 'ssrb'
                            % Rebinning ssrb.
                            zDirectos = ssrb(posDet{r, zm}(:,3), posDet{r, zm}(:,6));
                        case 'msrb' 
                            % Rebinning msrb.
                    end
                    ...
                    sinograms2D{r,zm} = sinograms2D{r,zm} + hist4([theta r_sino zDirectos], ...
