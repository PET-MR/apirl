...
function [sinograms2D, TiempoSimulacion] = getSinograms2D(outputPath, structSimu, energyWindow, structSizeSino2D, resEspacialFwhm_mm, rebinningMethod, zonaMuerta_mm, graficarOnline)
...
for r = 1 : numel(resEspacialFwhm_mm)
    for zm = 1 : size(zonaMuerta_mm,1)
        sinograms2D{r,zm} = zeros(structSizeSino2D.numTheta, structSizeSino2D.numR, ...
                ...
                for r = 1 : numel(resEspacialFwhm_mm)   % Por ahora considero que la resolución espacial es la misma en los dos sentidos.
                    for zm = 1 : size(zonaMuerta_mm, 1)
                        ...
                        switch(rebinningMethod)
                            case 'ssrb'
                                % Rebinning ssrb.
                                posDet{r, zm} = ssrb(posDet{r, zm});
                            case 'msrb' 
                                % Rebinning msrb.
                                posDet{r, zm} = msrb(posDet{r, zm}, structSizeSino2D.rFov_mm, structSizeSino2D.zFov_mm, structSizeSino2D.numZ);
                        end
                        ...
                        sinograms2D{r,zm} = sinograms2D{r,zm} + hist4([theta r_sino posDet{r, zm}(:,3)], ...
...
for r = 1 : numel(resEspacialFwhm_mm)   % Por ahora considero que la resolución espacial es la misma en los dos sentidos.
    for zm = 1 : size(zonaMuerta_mm, 1)
        filename = sprintf('%s/sinograms2d_res%.2f_zm%.2f_%.2f_%s', outputPath, resEspacialFwhm_mm(r), zonaMuerta_mm(zm), rebinningMethod);
