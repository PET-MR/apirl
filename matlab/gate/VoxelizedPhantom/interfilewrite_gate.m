...
function interfilewrite_gate(image, filename, sizePixel, relativePathForBinary)

if nargin < 4
    relativePathForBinary = [];
end
...
if ~isempty(barras)
    if isempty(relativePathForBinary)
        filenameImageForHeader = filenameImage(barras(end)+1 : end);
    else
        filenameImageForHeader = ['./' relativePathForBinary '/' filenameImage(barras(end)+1 : end)];
    end
else
    if isempty(relativePathForBinary)
        filenameImageForHeader = filenameImage;
    else
        filenameImageForHeader = ['./' relativePathForBinary '/' filenameImage];
    end
end
...
if(numel(sizePixel)==3)
    fprintf(fid,'slice thickness (pixels) := %d\n', sizePixel(3));
    fprintf(fid,'centre-centre slice separation (pixels) := %d\n', sizePixel(3));
else
