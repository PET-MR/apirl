...
numDiff = sum(sum(single(sinoEfficienciesSystemMatrix) ~= sinoEfficencies));

disp(sprintf('Hay %d bins diferentes en los sinogramas.', numDiff));
%% TRANSPOSE TO GET THE EFFICENCIES FROM THE SINOGRAM
effDet1 = detector1SystemMatrix'*double(sinoEfficencies(:)) ./ (sum(detector1SystemMatrix',2));
effDet1(isnan(effDet1)) = 0;
% figure;
% plot(effDet1);
% title('Estimated efficencies from Tranpose of System Matrix 1');
effDet2 = detector2SystemMatrix'*double(sinoEfficencies(:)) ./ (sum(detector2SystemMatrix',2));
...
%% MATRIX VERSION
% Forward:
forwardSino = diag(detector1SystemMatrix * crystalEffAllRings)*(crystalEffAllRings'*detector2SystemMatrix');
%% MATRIX VERSION 2
forwardSino2 = detector1SystemMatrix*(crystalEffAllRings*crystalEffAllRings');