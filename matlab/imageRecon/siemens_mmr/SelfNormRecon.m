...
function [volume, crystalEfficiencies] = SelfNormRecon(siemensSinogramFilename, normFilename, attMapBaseFilename, randomsFilename, scatterFilename, outputPath, pixelSize_mm, numOsemSubsets, numOsemIterations, numXtalIterations, filenameSystemMatrix1, filenameSystemMatrix2, stirMatlabPath, applyNormMask)
...
if isstr(attMapBaseFilename)
    ...
    acfFilename = '';
    acfsSinogram = ones(size(sinogram));
end
...
for iteration = 1 : numXtalIterations
    ...
    [volume overall_ncf_3d acfsSinogram randoms scatter] = OsemMmr(siemensSinogramFilename, span, overall_ncf_3d, attMapBaseFilename, randomsFilename, scatterFilename, outputPathIter, pixelSize_mm, numOsemSubsets, numOsemIterations, saveInterval, useGpu, stirMatlabPath, removeTempFiles);
    ...
    % Compute likelihood:
    log_likelihood(iteration) = sum(sinogram(:).*log(projectedImage(:))-projectedImage(:));
