...
function sinograms2D = getSinogramsFromCoincidenceMatrix(filenameCoincidenceMatrix, outputFilename, structSizeSino2D, rebinningMethod, energyWindow, widthScatterWindows_TripleWindow)
...
while(~feof(fid))
    ...
    %% SINOGRAMA CON REBINNING
    if strcmp(rebinningMethod, 'ssrb')
        matrizEventos = ssrb(matrizEventos);
        matrizEventosScatter = ssrb(matrizEventosScatter);
    ...
    elseif strcmp(rebinningMethod, 'msrb')
        ...
        % Elijo la matriz de eventos
    else
        error('Método de rebinning inexistente');
    end
    
end

disp(sprintf('Porcentaje de Eventos que se eliminaron por estar fuera del Fov: %d', 100*sum(sum(sinograms2D))/eventosTotales));
% Sinograma corregido:
...
interfileWriteSino(sinograms2Dscatter, [outputFilename '_scatter']);
interfileWriteSino(sinograms2Dcorregido, [outputFilename '_scatterCorrected']);