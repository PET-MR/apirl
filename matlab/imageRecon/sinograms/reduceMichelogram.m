...
function sinogram3D = reduceMichelogram(michelogram, sinogramsPerSegment, minRingDiff, maxRingDiff)

sinogram3D = [];

if(size(sinogramsPerSegment)~=size(minRingDiff))|(size(sinogramsPerSegment)~=size(maxRingDiff))
...
% + 1:
if( indiceSino ~= (sum(sinogramsPerSegment)+1))
