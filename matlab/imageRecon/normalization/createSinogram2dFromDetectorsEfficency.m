...
%  *********************************************************************
%  function sinoEfficencies = createSinogram2dFromDetectorsEfficency(efficenciesPerDetector, structSizeSino2d)
%
...
for det1 = 1 : numel(efficenciesPerDetector)
    for indDet2 = 1 : structSizeSino2d.numR/2
        ...
        det2 =  mod((det1 - 2*indDet2 + (numDetectors-structSizeSino2d.numR)/2), numDetectors)+1;
        ...
        det2 = mod((det1 - 2*indDet2 + 1 + (numDetectors-structSizeSino2d.numR)/2), numDetectors)+1;
        if (indProj > 0) && (det2 > 0) && (indProj <= structSizeSino2d.numTheta) && (det2 <= numDetectors)
            histDetIds = histDetIds + hist([det1; det2], detectorIds);
