...
//    */
    ...
    virtual bool Backproject (float * d_inputSinogram, float* d_outputImage, Sinogram3DCylindricalPet* inputSinogram, Image* outputImage, bool copyResult); 
    /** DivideAndBackprojection con Siddon para Sinogram3D. */
    virtual bool DivideAndBackproject (float* d_inputSinogram, float* d_estimatedSinogram, float* d_outputImage, Sinogram3DCylindricalPet* inputSinogram, Image* outputImage, bool copyResult);
    /** Projection con Siddon para Sinogram3D. */
    virtual bool Project(float* d_image, float* d_projection, Image* inputImage, Sinogram3DCylindricalPet* outputSinogram, bool copyResult);
    
    /// Método que incializa parámetros del proyector en la memoria de GPU.
    virtual void initGpuMemory(Sinogram3D* inputSinogram);
};
