...
__global__ void cuSiddonOversampledProjection (float* volume, float* michelogram, float *d_ring1, float *d_ring2, int numR, int numProj, int numRings, int numSinos, int numSamples, int numAxialSamples)
{
  int iBin =  threadIdx.x + (blockIdx.x * blockDim.x);
  // First compare inside the sinogram:
  if(iBin>= d_numBinsSino2d)
  ...
  iBin = iBin + blockIdx.y * d_numBinsSino2d;
  // axial step:
  float deltaZ = (d_ring1[1]-d_ring1[0])/numAxialSamples;
  ...
  for(int i = 0; i < numSamples; i++)
  {
	  for(int j = 0; j < numAxialSamples; j++)
	  {
		...
		//CUDA_GetPointsFromLOR(d_thetaValues_deg[iProj], rLimInf, d_ring1[blockIdx.y]-d_ringSize_mm/2+deltaZ/2+j*deltaZ, d_ring2[blockIdx.y]-d_ringSize_mm/2+deltaZ/2+j*deltaZ, d_RadioScanner_mm, &P1, &P2);
...
__global__ void cuSiddonOversampledDivideAndBackproject(float* d_inputSinogram, float* d_estimatedSinogram, float* d_outputImage, 
					     float *d_ring1, float *d_ring2, int numR, int numProj, int numRings, int numSinos, int numSamples, int numAxialSamples)
{
  float4 P1;
  float4 P2;
  float4 LOR;
...
__global__ void cuSiddonOversampledBackprojection(float* d_inputSinogram, float* d_outputImage, 
				       float *d_ring1_mm, float *d_ring2_mm, int numR, int numProj, int numRings, int numSinos, int numSamples, int numAxialSamples)
{
  float4 P1;
  float4 P2;
  float4 LOR;
  ...
  int iProj = (int)((float)iBin / (float)numR);
  // axial step:
  //float deltaZ = (d_ring1_mm[1]-d_ring1_mm[0])/numAxialSamples;
  iBin = iBin + blockIdx.y * d_numBinsSino2d;
  ...
  if(d_inputSinogram[iBin] != 0)
  {
	...
	for(int i = 0; i < numSamples; i++)
	{
		...
		{
			// The r coordinate is in rLimInf  
			//CUDA_GetPointsFromLOR(d_thetaValues_deg[iProj], rLimInf, d_ring1_mm[blockIdx.y]-d_ringSize_mm/2+deltaZ/2+j*deltaZ, d_ring2_mm[blockIdx.y]-d_ringSize_mm/2+deltaZ/2+j*deltaZ, d_RadioScanner_mm, &P1, &P2);
