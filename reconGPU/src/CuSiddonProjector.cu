...
bool CuSiddonProjector::Project (float* d_image, float* d_projection, Image* inputImage, Sinogram3DCylindricalPet* outputSinogram, bool copyResult)
{
  /* Este método simplemente encapsula la llamada al kernel.
    ...
    */
  cuSiddonProjection<<<gridSize, blockSize>>>(d_image, d_projection, outputSinogram->getNumR(), outputSinogram->getNumProj(), outputSinogram->getNumRings(), outputSinogram->getNumSinograms());
  /// Sincronización de todos los threads.
  cudaThreadSynchronize();
  return true;
}

bool CuSiddonProjector::DivideAndBackproject (float* d_inputSinogram, float* d_estimatedSinogram, float* d_outputImage, Sinogram3DCylindricalPet* inputSinogram, Image* outputImage, bool copyResult)
{
  /* Este método simplemente encapsula la llamada al kernel.
    El tamaño de la ejecución del kernel está definida en las propiedades gridSize y blockSize de la clase.
    La misma se configura en el constructor o con el método setKernelConfig.
    */
  //CUDA_Forward_Projection<<<gridSize, blockSize>>>(inputImage, float* michelogram, float* michelogram_measured, inputSinogram->, int numProj, int numRings);
  /// Sincronización de todos los threads.
  cudaThreadSynchronize();
  return true;
}

bool CuSiddonProjector::Backproject (float * d_inputSinogram, float* d_outputImage, Sinogram3DCylindricalPet* inputSinogram, Image* outputImage, bool copyResult)
{
  /* Este método simplemente encapsula la llamada al kernel.
    El tamaño de la ejecución del kernel está definida en las propiedades gridSize y blockSize de la clase.
    La misma se configura en el constructor o con el método setKernelConfig.
