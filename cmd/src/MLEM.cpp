/**
	\file MLEM.cpp
	\brief Archivo ejecutable para comando de reconstrucción con MLEM.

	Este archivo genera el comando ejecutable para reconstrucción con el algoritmo MLEM. Para esto
	...
	\par Ejemplo de archivo de configuración de reconstrucción reconMLEM.par:
	\code
	    MLEM Parameters :=
	    ...
	    END :=
	\endcode
	\todo Agregar para guardar los valores de ML de cada iteración y que los guarde en un txt.
	      Se debería quitar el input type cuando se tenga una clase projection que contenga todos los tipos
	      de datos de entrada.
	      Como necesito generar resultados rápidos la implementación la hago para sinogramas 3D del tipo del
	      PET GE Advance. Luego extenderlo a sinogramas de distinto tamaño, para eso los sinogramas deben
	      tener un método que permita leer el intefile propietario. Se puede hacerlo tranquilamente con la
	      medcon al igual que para las imágenes.
	\bug
	\warning
...
int main (int argc, char *argv[])
{
  ...
  if(strForwardprojector.compare("Siddon") == 0)
  {
	...
	if(getSiddonProjectorParameters(parameterFileName, "MLEM", &numSamples, &numAxialSamples))
  ...
  else if(inputType.compare("Sinogram2DArPet")==0)
  {
    ...
    // No se si los necesito porque en principio van a ser fijos los parámetros del scanner.	
//     if (getArPetScannerParameters(parameterFileName, &radiusFov_mm, &zFov_mm, &radiusScanner_mm))
//       return -1;
    // "diameter_of_colimator (in mm)"
    /*if((errorCode=parametersFile_read((char*)parameterFileName.c_str(), "MLEM", "diameter_of_colimator (in mm)", returnValue, errorMessage)) != 0)
    {
	    // Hubo un error. Salgo del comando.
	    if(errorCode == PMF_KEY_NOT_FOUND)
	    {
    ...
    float widthCollimator_mm = atoi(returnValue);*/
    /*
    ...
    mlem = new Mlem2dTgs(inputProjection, initialEstimate, "", outputPrefix, numIterations, saveIterationInterval, bSensitivityFromFile, forwardprojector, backprojector);
    if(bSensitivityFromFile)
    {
	  mlem->setSensitivityFilename(sensitivityFilename);
    }
    if(attenMapFilename.length() != 0)
    {
	  // Se brindó un mapa de atenuación, lo cargo en el Mlem:
	  mlem->setAttenuationImage(attenMapFilename);
    }
    mlem->enableSaveIntermediates(saveIntermediateData);*/
  }
