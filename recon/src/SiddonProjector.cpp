...
bool SiddonProjector::Project (Image* inputImage, Sinogram3D* outputProjection)
{
  ...
  int numZ;
  float z1_mm, z2_mm;
  float ringWidth_mm = outputProjection->getAxialFoV_mm()/outputProjection->getNumRings();
float deltaZ_oversampling = ringWidth_mm/numAxialSamplesOnDetector;
  ...
  for(int i = 0; i < outputProjection->getNumSegments(); i++)
  {
	...
	z2_mm = (outputProjection->getSegment(i)->getSinogram2D(j)->getAxialValue2FromList(0)  + 
      ...
      {
		int k, l, n, o, p, LengthList;
		#pragma omp parallel private(k, l, LOR, MyWeightsList, LengthList, n, o, p, P1, P2, geomFactor)
		{
			MyWeightsList = (SiddonSegment**)malloc(sizeof(SiddonSegment*));
			#pragma omp for
			for(k = 0; k < outputProjection->getSegment(i)->getSinogram2D(j)->getNumProj(); k++)
			{
				for(l = 0; l < outputProjection->getSegment(i)->getSinogram2D(j)->getNumR(); l++)
				{
					...
					{
						for(p = 0; p < numAxialSamplesOnDetector; p++)
						{
							...
							{
								lorOk = outputProjection->getSegment(i)->getSinogram2D(j)->getPointsFromOverSampledLor(k,l,o,numSamplesOnDetector,m, ringWidth_mm, p, numAxialSamplesOnDetector, &P1, &P2, &geomFactor);
							}
							// If use axial compression, use the average z coordinate:
							if(!useMultipleLorsPerBin)
							{
								P1.Z = z1_mm -ringWidth_mm/2 +deltaZ_oversampling/2+p*deltaZ_oversampling; //P1.Z = z1_mm;
								P2.Z = z2_mm -ringWidth_mm/2 +deltaZ_oversampling/2+p*deltaZ_oversampling; //P2.Z = z2_mm;	
							}
							if(lorOk)
							{
								LOR.P0 = P1;
...
bool SiddonProjector::Backproject (Sinogram3D* inputProjection, Image* outputImage)
{
	...
	// Puntero a los píxeles:
	float* ptrPixels = outputImage->getPixelsPtr();
	int i, j, k, l, n, m, o, p, LengthList;
	...
	float z1_mm, z2_mm;
	int lorOk;
	float ringWidth_mm = inputProjection->getAxialFoV_mm()/inputProjection->getNumRings();
	float deltaZ_oversampling = ringWidth_mm/numAxialSamplesOnDetector;
	...
	{    
		...
		#pragma omp parallel private(k, l, m, o, p, LOR, P1, P2, MyWeightsList, LengthList, n, newValue, indexPixel, geomFactor, lorOk) shared(inputProjection,ptrPixels, outputImage, sizeImage)
		{
			MyWeightsList = (SiddonSegment**)malloc(sizeof(SiddonSegment*));
			#pragma omp for    
						...
						for(m = 0; m < numZ; m++)
						{
							...
							for(o = 0; o < numSamplesOnDetector; o++)
							{
								for(p = 0; p < numAxialSamplesOnDetector; p++)
								{
									...
									{
										lorOk = inputProjection->getSegment(i)->getSinogram2D(j)->getPointsFromOverSampledLor(k,l,o,numSamplesOnDetector,m, ringWidth_mm, p, numAxialSamplesOnDetector, &P1, &P2, &geomFactor);
									}
									// If use axial compression, use the average z coordinate:
									if(!useMultipleLorsPerBin)
									{
...
bool SiddonProjector::DivideAndBackproject (Sinogram3D* InputSinogram3D, Sinogram3D* EstimatedSinogram3D, Image* outputImage)
{
  ...
  int numZ;
  float z1_mm, z2_mm;
  float ringWidth_mm = InputSinogram3D->getAxialFoV_mm()/InputSinogram3D->getNumRings();
  float deltaZ_oversampling = ringWidth_mm/numAxialSamplesOnDetector;
  #pragma omp parallel private(i, j, k, l, m, o, p, LOR, P1, P2, MyWeightsList, LengthList, n, newValue, indexPixel, geomFactor) shared(InputSinogram3D,EstimatedSinogram3D,ptrPixels)
  {
    MyWeightsList = (SiddonSegment**)malloc(sizeof(SiddonSegment*));
	      ...
	      for(m = 0; m < numZ; m++)
	      {
			...
			for(o = 0; o < numSamplesOnDetector; o++)
			{
				for(p = 0; p < numAxialSamplesOnDetector; p++)
				{
					...
					/// sinograma, ya que varias LORS deben aportar al mismo bin del sinograma.
					int lorOk;
					if((numSamplesOnDetector == 1)&&(numAxialSamplesOnDetector == 1))
					{
						lorOk = InputSinogram3D->getSegment(i)->getSinogram2D(j)->getPointsFromLor(k,l,m, &P1, &P2, &geomFactor);
					}
					else
					{
						lorOk = InputSinogram3D->getSegment(i)->getSinogram2D(j)->getPointsFromOverSampledLor(k,l,o,numSamplesOnDetector,m, ringWidth_mm, p, numAxialSamplesOnDetector, &P1, &P2, &geomFactor);
					}
					// If use axial compression, use the average z coordinate:
					if(!useMultipleLorsPerBin)
					{
